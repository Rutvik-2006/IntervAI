...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!token) {
      setFormErrors({ general: 'Reset token is missing from URL.' });
      return;
    }
        ...
        <div className="rounded-2xl border border-slate-900 bg-slate-900/40 p-8 backdrop-blur-xl shadow-2xl">
          {(error || formErrors.general) && (
            <div className="mb-6 flex items-start space-x-3 rounded-lg bg-rose-500/10 border border-rose-500/20 p-4 text-rose-500">
              ...
              <p className="text-sm font-medium">{error || formErrors.general}</p>
            </div>
          )}

          {!token ? (
            <div className="text-center py-4">
              <p className="text-sm text-rose-500 font-semibold mb-4">
                No token found. Please check your recovery email link.
              </p>
              <Link to="/login">
